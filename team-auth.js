(function () {
  ...
  function initLogin() {
    ...
    form.addEventListener('submit', (event) => {
      ...
      App.toast('Signed in', `Opening the ${role === 'driver' ? 'driver console' : 'operations dashboard'}.`, 'success', 1000);
      setTimeout(() => { location.href = role === 'driver' ? 'driver.html' : 'admin.html'; }, 320);
    });
  }
  ...
  function initConsoleIdentity() {
    ...
    document.getElementById('driverLogout')?.addEventListener('click', () => { App.auth.clearSession(); location.href = 'login.html'; });
    document.getElementById('adminLogout')?.addEventListener('click', () => { App.auth.clearSession(); location.href = 'login.html'; });
  }

  document.addEventListener('DOMContentLoaded', () => { initLogin(); initConsoleIdentity(); });
})();
